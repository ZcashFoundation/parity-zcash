...
impl MemoryDatabase {
    pub fn drain_transaction(&self) -> Transaction {
        ...
        let sprout_nullifiers = replace(&mut db.sprout_nullifiers, HashMap::default())
            .into_iter()
            .flat_map(|(key, state)| {
                state.into_operation(
                    key,
        ...
        let sapling_nullifiers = replace(&mut db.sapling_nullifiers, HashMap::default())
            .into_iter()
            .flat_map(|(key, state)| {
                state.into_operation(
                    key,
        ...
        let sprout_tree_state = replace(&mut db.sprout_tree_state, HashMap::default())
            .into_iter()
            .flat_map(|(key, state)| {
                state.into_operation(key, KeyValue::SproutTreeState, |k| {
                    Key::TreeRoot(EpochRef::new(EpochTag::Sprout, k))
                })
            });

        let sprout_block_root = replace(&mut db.sprout_block_root, HashMap::default())
            .into_iter()
            .flat_map(|(key, state)| {
                state.into_operation(key, KeyValue::SproutBlockRoot, Key::SproutBlockRoot)
            });

        let sapling_tree_state = replace(&mut db.sapling_tree_state, HashMap::default())
            .into_iter()
            .flat_map(|(key, state)| {
                state.into_operation(key, KeyValue::SaplingTreeState, |k| {
                    Key::TreeRoot(EpochRef::new(EpochTag::Sapling, k))
                })
            });
            ...
            match op {
                Operation::Insert(insert) => match insert {
                    ...
                    KeyValue::Nullifier(key) => match key.epoch() {
                        EpochTag::Sprout => {
                            db.sprout_nullifiers
                                .insert(*key.hash(), KeyState::Insert(()));
                        }
                        EpochTag::Sapling => {
                            db.sapling_nullifiers
                                .insert(*key.hash(), KeyState::Insert(()));
                        }
                    },
                    KeyValue::SproutTreeState(key, value) => {
                        db.sprout_tree_state.insert(key, KeyState::Insert(value));
                    }
                    KeyValue::SaplingTreeState(key, value) => {
                        db.sapling_tree_state.insert(key, KeyState::Insert(value));
                    }
                    KeyValue::SproutBlockRoot(key, value) => {
                        db.sprout_block_root.insert(key, KeyState::Insert(value));
                    }
                },
                Operation::Delete(delete) => match delete {
                    ...
                    Key::Nullifier(key) => match key.epoch() {
                        EpochTag::Sprout => {
                            db.sprout_nullifiers.insert(*key.hash(), KeyState::Delete);
                        }
                        EpochTag::Sapling => {
                            db.sapling_nullifiers.insert(*key.hash(), KeyState::Delete);
                        }
                    },
                    Key::TreeRoot(key) => match key.epoch() {
                        EpochTag::Sprout => {
                            db.sprout_tree_state.insert(*key.hash(), KeyState::Delete);
                        }
                        EpochTag::Sapling => {
                            db.sapling_tree_state.insert(*key.hash(), KeyState::Delete);
                        }
                    },
                    Key::SproutBlockRoot(key) => {
                        db.sprout_block_root.insert(key, KeyState::Delete);
                    }
                },
            }
        }
        ...
        let result = match *key {
            ...
            Key::TreeRoot(ref key) => match key.epoch() {
                EpochTag::Sprout => db
                    ...
                    .cloned()
                    .unwrap_or_default()
                    .map(Value::SproutTreeState),
                EpochTag::Sapling => db
                    .sapling_tree_state
                    .get(key.hash())
                    .cloned()
                    .unwrap_or_default()
                    .map(Value::SaplingTreeState),
            },
            Key::SproutBlockRoot(ref key) => db
                .sprout_block_root
                .get(key)
                .cloned()
                .unwrap_or_default()
                .map(Value::SproutTreeRoot),
        };
