        ...
        if self.left.is_none() {
            ...
            for i in 0..D::HEIGHT - 1 {
                ...
                match parent_slot.take() {
                    ...
                    Some(former) => combined = H::combine(&former, &combined, i + 1),
                }
            }
    ...
    pub fn root(&self) -> H256 {
        ...
        for i in 0..D::HEIGHT - 1 {
            match &self.parents[i as usize] {
                &Some(ref parent) => {
                    root = H::combine(&parent, &root, i + 1);
                }
                &None => {
                    root = H::combine(&root, &H::empty()[i as usize + 1], i + 1);
                }
            }
        }
...
mod tests {
    ...
    fn commitment_1() {
        ...
        tree.append(H256::from_reversed_str(
            "bab6e8992959caf0ca94847c36b4e648a7f88a9b9c6a62ea387cf1fb9badfd62",
        ))
        .unwrap();
    ...
    fn commitments_full() {
        ...
        for i in 0..TEST_COMMITMENTS.len() {
            tree.append(TEST_COMMITMENTS[i].clone())
                .expect(&format!("Failed to add commitment #{}", i));
    ...
    fn sapling_empty_root() {
        let expected_root = H256::from_reversed_str(
