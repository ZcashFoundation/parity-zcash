...
pub trait VerificationSink:
    HeadersVerificationSink + BlockVerificationSink + TransactionVerificationSink
{
}
        ...
        while let Some(task) = tasks_queue.pop_front() {
            match task {
                VerificationTask::VerifyHeaders(peer, headers) => {
                    let result = headers.iter().try_for_each(|header| {
                        verifier
                    ...
                    match result {
                        ...
                        Err((error, hash)) => sink.on_headers_verification_error(
                            peer,
                            format!("{:?}", error),
                            hash,
                            headers,
...
pub mod tests {
    ...
    fn verification_level_header_accept_incorrect_transaction() {
        ...
        assert_eq!(
            wrapper.verify_block(&bad_transaction_block.clone().into()),
            Ok(())
        );
        ...
        assert_eq!(
            wrapper.verify_block(&bad_transaction_block.into()),
    ...
    fn verification_level_none_accept_incorrect_block() {
        ...
        assert_eq!(
            wrapper.verify_block(&bad_block.into()),
