    ...
    F: Invoke<(H256, chain::Block)>,
{
    ...
    pub fn build(self) -> F::Result {
        let block = self
            .block
            .expect("Block is supposed to be built here to get hash");
    ...
    F: Invoke<chain::TransactionOutput>,
{
    ...
    pub fn founder_reward(mut self, consensus: &ConsensusParams, height: u32) -> Self {
        self.script_pubkey =
            ScriptBuilder::build_p2sh(&consensus.founder_address(height).unwrap().hash).into();
