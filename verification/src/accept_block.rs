...
impl<'a> BlockFounderReward<'a> {
    ...
    fn check(&self) -> Result<(), Error> {
        if let Some(ref founder_address) = self.founder_address {
            ...
            let has_founder_reward = self
                .block
                .transactions
                .first()
                .map(|tx| {
                    tx.raw.outputs.iter().any(|output| {
                        **output.script_pubkey == *script && output.value == self.founder_reward
                    })
                })
                .unwrap_or(false);
    ...
    fn check(&self) -> Result<(), Error> {
        ...
        let mut sapling_tree = if self.block.header.raw.previous_header_hash.is_zero() {
            ...
            self.tree_state_store
                .sapling_tree_at_block(&self.block.header.raw.previous_header_hash)
        ...
        for tx in &self.block.transactions {
            if let Some(ref sapling) = tx.raw.sapling {
                for out in &sapling.outputs {
                    sapling_tree
                        .append(out.note_commitment.into())
...
mod tests {
    ...
    fn test_block_sapling_root() {
        ...
        assert_eq!(
            ...
            .check(),
            Err(Error::InvalidFinalSaplingRootHash {
    ...
    fn test_coinbase_overspend_b419221() {
        ...
        assert_eq!(
            BlockCoinbaseMinerReward::new(
                CanonBlock::new(&block.into()),
                &store,
                &consensus,
                419221
            )
