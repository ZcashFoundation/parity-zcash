...
impl<'a> TransactionAcceptor<'a> {
    pub fn new(
        ...
        TransactionAcceptor {
            ...
            join_split: JoinSplitVerification::new(
                consensus,
                transaction,
                nullifier_tracker,
                tree_state_provider,
...
impl<'a> TransactionBip30<'a> {
    fn new_for_sync(transaction: CanonTransaction<'a>, store: &'a TransactionMetaProvider) -> Self {
        TransactionBip30 {
...
impl<'a> TransactionOverspent<'a> {
    ...
    fn check(&self) -> Result<(), TransactionError> {
        ...
        checked_transaction_fee(&self.store, ::std::usize::MAX, &self.transaction.raw).map(|_| ())
    }
}
...
impl<'a> TransactionEval<'a> {
    ...
    fn check(&self) -> Result<H256, TransactionError> {
        ...
        let require_no_input_sighash =
            self.transaction.raw.join_split.is_some() || self.transaction.raw.sapling.is_some();
        ...
        if self
            .verification_level
            .intersects(VerificationLevel::HEADER | VerificationLevel::NO_VERIFICATION)
        {
...
mod tests {
    ...
    fn join_split() {
        ...
        assert_eq!(
            verify_script(&input_script, &output_script, &flags, &mut checker),
            Ok(())
        );
    }
    ...
    fn transaction_expiry_works() {
        ...
        assert_eq!(
            TransactionExpiry::new(
                CanonTransaction::new(&tx),
                &consensus,
                consensus.overwinter_height + 100
            )
            .check(),
    ...
    fn transaction_version_works() {
        ...
        assert_eq!(
            ...
            .check(),
            Err(TransactionError::InvalidOverwintered)
        );
