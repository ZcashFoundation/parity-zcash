...
pub trait ClientCore {
    ...
    fn on_headers(
        ...
        headers: Vec<IndexedBlockHeader>,
    ) -> Option<Vec<IndexedBlockHeader>>;
    fn on_block(
        &mut self,
        peer_index: PeerIndex,
        block: IndexedBlock,
    ) -> Option<VecDeque<PartiallyVerifiedBlock>>;
    fn on_transaction(
        &mut self,
        peer_index: PeerIndex,
    ...
    T: TaskExecutor,
{
    ...
    fn on_headers(
        ...
        assert!(
            !headers.is_empty(),
            "This is checked in incoming connection"
        );
            ...
            | BlockState::DeadEnd => {
                ...
                match parent_block_state {
                    ...
                    BlockState::VerifyingHeader | BlockState::Verifying | BlockState::Stored => {
                        ...
                        let orphaned_blocks = self
                            .orphaned_blocks_pool
                            .remove_blocks_for_parent(&block.header.hash);
                        blocks_to_verify_hashes
                            .extend(orphaned_blocks.iter().map(IndexedBlock::hash).cloned());
                        self.chain
                            .forget_blocks_leave_header(&blocks_to_verify_hashes);
                        // remember that we are verifying these blocks
                        let blocks_to_verify = ::std::iter::once(block)
                            .chain(orphaned_blocks)
                            .map(|block| {
                                if self.chain.verify_block(block.header.clone()) {
                        ...
                        match self.verifying_blocks_futures.entry(peer_index) {
                            ...
                            Entry::Vacant(entry) => {
                                entry.insert((
                                    blocks_to_verify_hashes.into_iter().collect(),
                                    Vec::new(),
                                ));
                            }
                        }
            ...
            .collect();
        {
            ...
            if num_headers_idle_peers > 0 {
                ...
                if scheduled_hashes_len < MAX_SCHEDULED_HASHES {
                    let mut headers_idle_peers: Vec<_> = self
                        .peers_tasks
                    ...
                    let num_headers_idle_peers =
                        ::std::cmp::min(num_headers_idle_peers, headers_idle_peers.len());
    ...
    fn try_switch_to_saturated_state(&mut self) -> bool {
        let switch_to_saturated = {
            ...
            self.chain
                .length_of_blocks_state(BlockState::VerifyingHeader)
                == 0
...
impl<T> HeadersVerificationSink for CoreVerificationSink<T>
where
    T: TaskExecutor,
{
    ...
    fn on_headers_verification_error(
        ...
        self.core
            .lock()
            .on_headers_verification_error(peer, error, hash, headers)
    }
}
    ...
    T: TaskExecutor,
{
    ...
    fn find_unknown_headers(
        &self,
        mut headers: Vec<IndexedBlockHeader>,
        ...
        if first_unknown_index == 0 {
            ...
            headers.split_off(first_unknown_index)
        }
    }
    ...
    fn compute_num_peers_for_headers_request(&self) -> usize {
        ...
        if precise_time_s() - self.new_headers_receival_timestamp
            > MIN_HEADERS_DUPLICATION_INTERVAL_S
        {
...
pub mod tests {
    ...
    fn known_blocks_are_ignored_in_headers_verification_success() {
        ...
        sync.orphaned_blocks_pool
            .insert_unknown_block(block2.clone());
        ...
        assert_eq!(
            sync.chain().block_state(&hash1),
            BlockState::VerifyingHeader
        );
